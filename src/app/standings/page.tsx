    ...
    return (
        <div className="space-y-6">
            ...
            <div className="rounded-2xl bg-white/5 border border-white/10 overflow-x-auto">
                <div className="grid grid-cols-[auto_1fr_50px_50px_50px_60px_60px] md:grid-cols-[auto_1fr_60px_60px_60px_80px_80px] gap-3 md:gap-4 px-3 md:px-6 py-3 border-b border-white/10 text-xs text-gray-500 uppercase tracking-wider font-medium min-w-fit">
                ...
                {leagueTeams.map((team, idx) => (
                    <div key={team.id}>
                        <div
                            className={`grid grid-cols-[auto_1fr_50px_50px_50px_60px_60px] md:grid-cols-[auto_1fr_60px_60px_60px_80px_80px] gap-3 md:gap-4 px-3 md:px-6 py-3 md:py-4 items-center hover:bg-white/5 cursor-pointer transition-all min-w-fit ${idx < 3 ? 'border-l-2 border-l-amber-500/50' : ''}`}
                            onClick={() => setExpandedTeam(expandedTeam === team.id ? null : team.id)}
                        >
                            <span className={`w-6 md:w-8 text-xs md:text-sm font-bold ${idx === 0 ? 'text-amber-400' : idx < 3 ? 'text-amber-400/60' : 'text-gray-500'}`}>
                            ...
                            <div className="flex items-center gap-2 md:gap-3 min-w-0">
                                ...
                                <div className="flex items-center gap-1 md:gap-2 min-w-0">
                                    <span className="font-semibold text-white text-xs md:text-sm truncate">{team.team_name}</span>
                            ...
                            <span className="text-center text-xs md:text-sm text-gray-300">{team.played}</span>
                            <span className="text-center text-xs md:text-sm text-emerald-400 font-medium">{team.won}</span>
                            <span className="text-center text-xs md:text-sm text-red-400 font-medium">{team.lost}</span>
                            ...
                            <span className="text-center text-xs md:text-sm font-black text-white">{team.points}</span>
                        </div>

                        {expandedTeam === team.id && (
                            <div className="px-3 md:px-6 pb-3 md:pb-4 grid grid-cols-1 sm:grid-cols-3 gap-2 md:gap-3">
                                <div className="p-2 md:p-3 rounded-xl bg-white/5 border border-white/5">
                                    <div className="text-[10px] md:text-xs text-gray-500">Stadium</div>
                                    <div className="text-xs md:text-sm text-white font-medium truncate">{team.stadium || 'N/A'}</div>
                                    <div className="text-[10px] md:text-xs text-gray-500">{team.capacity?.toLocaleString() || '—'} seats</div>
                                </div>
                                <div className="p-2 md:p-3 rounded-xl bg-white/5 border border-white/5">
                                    <div className="text-[10px] md:text-xs text-gray-500">Budget</div>
                                    <div className="text-xs md:text-sm text-white font-medium">{formatMoney(team.team_money)}</div>
                                </div>
                                <div className="p-2 md:p-3 rounded-xl bg-white/5 border border-white/5">
                                    <div className="text-[10px] md:text-xs text-gray-500">Win Rate</div>
                                    <div className="text-xs md:text-sm text-white font-medium">{team.played > 0 ? Math.round((team.won / team.played) * 100) : 0}%</div>
                                </div>
                            </div>
                        )}
                    </div>
